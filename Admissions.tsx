...
import { FileText, CalendarCheck, CheckCircle, Smartphone, Download, QrCode } from 'lucide-react';
...
const Admissions: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Optional: keep your email trigger here to capture the lead immediately
      // We use a shorter timeout or ignore errors so navigation isn't blocked
      fetch("/send-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      }).catch(err => console.log("Background lead capture failed, proceeding to next page."));

      // 3. REDIRECT to the detailed digital enrolment page
      // We pass the current formData in the 'state' object so the next page is pre-filled
      navigate('/enrolment', { state: { initialData: formData } });
      
    } catch (error) {
      console.error("Navigation Error:", error);
      // Fallback: navigate anyway so the user experience isn't broken
      navigate('/enrolment', { state: { initialData: formData } });
    }
  };
          ...
          <div className="lg:w-1/2 space-y-6">
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-dashed border-white/20 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm font-black text-orange-200">96036 23494 | 90008 52892</p>
              </div>
            </div>
          </div>

          {/* Right Side Form (Step 1) */}
          <div className="lg:w-1/2 w-full bg-white text-slate-900 p-6 md:p-8 rounded-3xl shadow-xl border-t-4 border-orange-600">
            <div className="mb-6 text-center">
              <h4 className="text-xl font-black mb-1 uppercase tracking-wider">Online Enrolment</h4>
              {/* <p className="text-slate-500 text-xs font-bold uppercase tracking-widest opacity-60">Step 1: Express Interest</p> */}
