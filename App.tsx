...
// --- IMPORT YOUR DEDICATED CAMPUS FILES ---
import DefenceAcademy from './components/DefenceAcademy';
import OrionBranch1 from './components/OrionBranch1';
import OrionBranch2 from './components/OrionBranch2';
import JuniorBoys from './components/JuniorBoys';
import JuniorGirls from './components/JuniorGirls';
import DegreeCollege from './components/DegreeCollege';
import VocationalCollege from './components/VocationalCollege';

import { MessageSquare, ChevronUp, Star, Megaphone } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen selection:bg-orange-100 selection:text-orange-900 bg-white">
      ...
      <Routes>
                  ...
                  {TESTIMONIALS.map((t, i) => (
                    <div key={i} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm relative group hover:shadow-xl transition-all duration-500">
                      <div className="flex items-center gap-4 mb-6">
                        <img src={t.img} alt={t.name} className="w-14 h-14 rounded-full object-cover border-2 border-orange-100" />
                        <div>
                          <h4 className="font-black text-slate-900 text-sm uppercase">{t.name} [cite: 10]</h4>
                          <p className="text-[10px] font-bold text-orange-600 uppercase tracking-widest">{t.rank} </p>
                        </div>
                      </div>
                      <div className="flex gap-1 mb-4">
                      ...
                      <p className="text-slate-600 text-xs font-medium italic">"{t.text}"</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {FAQS.map((faq, idx) => (
                    <div key={idx} className="border border-slate-100 rounded-2xl overflow-hidden">
                      <button onClick={() => setFaqOpen(faqOpen === idx ? null : idx)} className="w-full flex items-center justify-between p-5 text-left bg-white">
                        <span className="font-bold text-sm text-slate-800 uppercase tracking-wide">{faq.question}</span>
                      </button>
                      {faqOpen === idx && <div className="px-5 pb-5 text-slate-600 text-sm font-medium border-t border-slate-50 pt-3 text-left">{faq.answer}</div>}
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* --- DEDICATED CAMPUS ROUTES --- */}
        <Route path="/campus-defence" element={<DefenceAcademy />} />
        <Route path="/campus-orion-1" element={<OrionBranch1 />} />
        <Route path="/campus-orion-2" element={<OrionBranch2 />} />
        <Route path="/campus-junior-boys" element={<JuniorBoys />} />
        <Route path="/campus-junior-girls" element={<JuniorGirls />} />
        <Route path="/campus-degree" element={<DegreeCollege />} />
        <Route path="/campus-vocational" element={<VocationalCollege />} />
      ...
      {/* Floating Buttons */}
      <div className="fixed bottom-24 lg:bottom-10 right-2 z-40 flex flex-col gap-2">
        <a href="#admissions" className="bg-[#e65100] text-white p-3 rounded-2xl shadow-2xl hover:bg-[#bf360c] transition-all transform hover:scale-110 active:scale-95 text-center">
          <Megaphone size={28} />
        </a>
        <a href="https://wa.me/919581540540" target="_blank" rel="noopener noreferrer" className="bg-green-500 text-white p-4 rounded-2xl shadow-2xl hover:bg-green-600 transition-all transform hover:scale-110 active:scale-95 group">
          <MessageSquare size={24} />
        </a>
        {showScrollTop && (
          <button onClick={scrollToTop} className="bg-slate-900 text-white px-4 py-1 rounded-2xl shadow-2xl hover:bg-orange-600 transition-all transform hover:scale-110">
            <ChevronUp size={24} />
          </button>
        )}
      </div>
